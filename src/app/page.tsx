  ...
  return (
    <main className={cx(
      ...
      <div className={css`position: relative; width: 39.259vh; height: 8.056vh;`}>
        {/* "I" */}
        <Line type="vertical" position={[0, 0]} startXY={[10, 0.25]} initialHeight={CAP_HEIGHT/2} transformOrigin="bottom left" />

        {/* "N" */}
        <Line type="vertical" position={[3.611, 0]} startXY={[8, -0.25]} initialHeight={CAP_HEIGHT/5}>
          <Line type="diagonal" position={[0, 0]} startXY={[0, 0]} initialHeight={CAP_HEIGHT/5} />
        </Line>
        ...
        {/* "I" */}
        <Line type="vertical" position={[16.435, 0]} startXY={[2, 0.25]} initialHeight={CAP_HEIGHT/2} transformOrigin="bottom left" />

        {/* "N" */}
        <Line type="vertical" position={[19.954, 0]} startXY={[-2, -0.25]} initialHeight={CAP_HEIGHT/2} >
          <Line type="diagonal" position={[0, 0]} startXY={[0, 0]} initialHeight={CAP_HEIGHT/5} />
        </Line>
        <Line type="vertical" position={[23.38, 0]} startXY={[-4, 0.25]} initialHeight={CAP_HEIGHT/2} transformOrigin="bottom left" />

        {/* "I" */}
        <Line type="vertical" position={[26.898, 0]} startXY={[-6, -0.25]} initialHeight={CAP_HEIGHT/3} />

        {/* "T" */}
        <Line type="vertical" position={[31.528, 0]} startXY={[-8, 0.25]} initialHeight={CAP_HEIGHT/5} transformOrigin="bottom left">
          <Line type="horizontal" width={3.472} position={[-1.528, 0]} startXY={[0, 0]} transformOrigin="top center" />
        </Line>

        {/* "E" */}
        <Line type="vertical" position={[36.065, 0]} startXY={[-10, -0.25]} initialHeight={CAP_HEIGHT * 0.667}>
